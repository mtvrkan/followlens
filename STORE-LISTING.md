...
- [x] `npm audit` clean
- [ ] **Publish the site**, so the privacy policy URL resolves — GitHub Pages,
      ...
      privacy policy.
- [ ] **Capture the five screenshots** (above)
...
- [x] Store id wired into `docs/index.html` — `jpejnlkciiphkcnlncljikpgekbcglfl`,
      in the install button's `href` and in the JSON-LD `downloadUrl`. Both link
      to a listing that is still in review, so the "Or install from source" note
      beneath the button has to stay until it is approved.
- [ ] Submit for review
- [ ] **Once approved:** drop the "in review" lines from `README.md`,
