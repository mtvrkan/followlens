...
assets/js/main.js             Header, back-to-top, scroll journey, reveals
assets/fonts/*.woff2          Inter + Manrope, self-hosted (see SITE-NOTES.md § 3)
assets/img/logo.svg           The brand mark, as served for the favicon
assets/img/og-image.png       The social card (see below)
CNAME                         Custom domain for GitHub Pages
```

No build step, no dependencies, no bundler, and no external requests at all —
the typefaces are served from this origin, which is what lets the CSP name
`default-src 'none'` with no outside host anywhere in it.

## Why any of it is the way it is

Everything in this folder is published byte for byte, so it carries no comments:
anyone can read them with view-source, and the page should ship only what the
browser needs. The reasoning lives in [`../SITE-NOTES.md`](../SITE-NOTES.md) —
tokens and the lighting model, every section's layout decisions, the scroll
journey, the i18n contract, and a list of things that were removed so they do
not get reinvented. **Change the code, change the note.**
